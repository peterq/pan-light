...
function enterPath(path, dontRecordHistory) {
    ...
    var promise = Util.callGoAsync('pan.ls', {path: path})
    appState.enterPathPromise = promise
    promise.finally(function(){appState.enterPathPromise = null})
    ...
    // 历史记录操作
    if (dontRecordHistory || path === appState.accessDirHistory[appState.accessDirHistoryIndex]) return
    if (appState.accessDirHistoryIndex !== appState.accessDirHistory.length -1) {
        for (var i = 0; i < appState.accessDirHistory.length - appState.accessDirHistoryIndex; i++) {
